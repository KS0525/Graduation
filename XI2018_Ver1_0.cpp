...
namespace XI
{
	...
	void  Pad::B_UpDate()
	{
		...
		SIUpdate(this->rs, state.Gamepad.sThumbRX * digToAnalog, state.Gamepad.sThumbRX  * digToAnalog);
		TGUpdate(this->tg, state.Gamepad.bLeftTrigger * digToAnalog_TG, state.Gamepad.bRightTrigger * digToAnalog_TG);
	...
	void  KeyBoard::B_UpDate()
	{
		...
		//トリガーボタン用に登録されたキーへの対応
		float  tg_L = 0, tg_R = 0;
		if (this->kbTrigger.size() == 2) {
			if (keyStatus[kbTrigger[0]] != 0) { tg_L = 1; }
			if (keyStatus[kbTrigger[1]] != 0) { tg_R = 1; }
		}
