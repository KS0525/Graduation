...
namespace ML
{
	...
	//図形クラスの基底クラス--------------------------------------------------------------------------
		...
		bool Shape::Hit(const Shape::SP  b_, ExtParam&  ex_)
		{
			...
			case  KIND::AABB:		return  Hit(*(static_cast<const  class  AABB*>(p)),ex_);	break;	//AABBとの接触
			case  KIND::Sphere:		return  Hit(*(static_cast<const  class  Sphere*>(p)),ex_);	break;	//球体との接触
			case  KIND::OBB:		return  Hit(*(static_cast<const  class  OBB*>(p)),ex_);		break;	//OBBとの接触
				//★図形を増やしたら追加★
			}
		}
	//AABB--------------------------------------------------------------------------
