...
public:
  bool QuitFlag() { return quitRequest; };
	...
	virtual ~GameEngine( );
	//	タスクの登録
	//注意！　１８年度以降、このメソッドを直接呼び出す必要はありません
	//xxxx::Object::Create(true);を呼び出した時点で、このメソッドも自動で呼び出されます。
	//その為、このメソッドを呼ぶとタスクの二重登録が起こり、UpDate(　) Render系( )が、
	//二度呼ばれるようになるため、バグの原因となります。
	//現在このバグを回避させるため、メソッド名を変な名前に変更してあります。
	void PushBack_ABCDEFGHIJKLMN(const BTask::SP& t_);
	//	同名のタスクを全て削除する
